...
We know that these are big high resolution images which can really drag down resources - especially for graphics.  We provide a simple way to resize a stars object.  This step always generates a warning from `GDAL` but it seems that we can ignore the warning without worry.
